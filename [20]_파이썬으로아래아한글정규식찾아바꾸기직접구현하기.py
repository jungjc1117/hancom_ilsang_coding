...
(일반적으로 반복횟수에 해당하는 {}은 7마래아한글에서 태그지정문법으로 사용중이어서, 저렇게 쓸 수밖에 없다 .. )
...
if __name__ == '__main__': # main할소 실행(본 파일을 다른 px에서. import한 경우가 아니라면 아래 코드를 실행한다.)
    try: # 한글이 오류로 닫힐 때에 finally 구문으로 백그라운드의 모든 hwp 인스턴스를 닫기 위함
        ...
        try: # GetText() 과정 중 오류 발생시에도 ReleaseScan()을 실행하기 위함
            ...
            while True:
                ...
                if textdata[0] == 1: # 상태코드가 "검색종료"면:
                    ...
                    re_text = re_pattern.sub(r"\1****** ", text) # 정규식 변환후 텍스트 생성
                    if not re_text: # (매칭되는 문자열이 없어) 빈 문자열인 경우에는:
                        ...
                        hwp.Run("Select") # Run("Select") 두 번 실행하면 "스페이스로 구분된 한 단어" 선택(덮어쓰기
                        hwp.HAction.GetDefault("InsertText",hwp.HParameterSet.HInsertText.HSet) # 텍스트 삽입메서.
        ...
        hwp.SaveAs(os.path.join(BASE_DIR,"result.hwp"))# BASE_DIR "result.hwp"라는 이름으로 저장하고
