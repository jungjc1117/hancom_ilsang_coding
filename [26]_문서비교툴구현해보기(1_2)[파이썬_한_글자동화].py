import difflib # 두 개 문자열간의 차이점을 분석하는 데 쓸 수 있는 외장 라이브러리입니다.설치되어 있어야 합니다.
...
def 글자색(Color): # 먼저 함수를 정의해놓겠습니다.
    ...
    hwp.HAction.Run(f"CharShapeTextColor{Color.capitalize()}") # 색변경 메서드를 실행 후 색을 바꿉니다.
...
if __name__ == '__main__': 
    ...
    hwp.Run("FileNew") # 새 창
    hwp.Open(r"C:\Users\jjc\Desktop\한컴오피스 업무자동화 입문\첨부파일\별헤는밤_원본.hwpx") # 원본 열고,
    hwp.Run("FileNew") # 또 새 창,
    hwp.Open(r"C:\Users\jjc\Desktop\한컴오피스 업무자동화 입문\첨부파일\별헤는밤_조작.hwpx") # 조작한 버전 열고,
    hwp.Run("FileNew") # 또 새 창,
    hwp.Open(r"C:\Users\jjc\Desktop\한컴오피스 업무자동화 입문\첨부파일\비교표.hwpx") # 비교표를 작성할 빈 문서입니다.
    ...
    # 저는 마우스 클릭이r 귀찮아서 그냥 한/글 창을 여러 개 만들어서 작업합니다.

    # 인덱스는 hwp.XHwpDocuments.Item(index)로 접근할 수 있는데,
    # 이 인덱스도 0부터 시작합니다.
    # 2019년 12월 현재 아래아한글 2018 버전 최신 업데이트시
    # 자동화 인스턴스를 생성하면 초기에 백그라운드에서 실행이 됩니다.
    # 이 백그라운드 인스턴스를 나타나게 하는 메서드가 아직 없는 상태여서 무시하고요
    # FileNew 액션으로 새 창을 열고 1번째 인덱스부터 사용을 하는 중입니다.
    # 현재 0번째 인덱스도 열려 있기는 합니다.
    # 이 점 때문에, 한글 자동화작업을 마쳤다고, 닫기버튼을 눌러서 한글을 꺼버리면
    # 백그라운드에서 실행중인 한글창 하나가 열려있는 상태가 돼버립니다.
    # 그래서 자동화를 마치면 무조건 hwp.Quit() 메서드를 실행해서 종료해 주셔야 합니다.
    # 꼭 기억해주세요.이렇게 종료 안 하시면 백그라운드에 한/글창이 여러 개 띄워져서 메모리와 프로세스를 잡아먹게 됩니다.
    ...
    for original_statement in original_full_text: # 원본 문장을 전부 순회하면서
        ...
        for copy_statement in copy_full_text:
            original_words = original_statement.split(' ', 1)
            
            
            ##############
            copy_words = copy_statement.split(' ', 1)
            if len(original_words) > 1 and len(copy_words) > 1:
                similarity = difflib.SequenceMatcher(None, original_words[1], copy_words[1]).ratio()
            else:
                similarity = 0
            coupled_dict[similarity] = copy_statement
        max_ratio = max(coupled_dict.keys())
        closest_match = coupled_dict[max_ratio].strip()
        cb.copy(closest_match) # 유사도가 제일 높은 문장을 찾아 복사하고,
            ###################
        ...
        if max_ratio < 1.0: # 완전히 똑같지 않다면?
            ...
            글자색('black') # 그렇지 않다면 다시 까맣게.(이건 한/글 버그 때문인데, 위쪽 셀의 글자가 빨갛게 되어 있으면, 아래쪽 셀의 글자도 빨갛게 되어 버립니다.그래서 이렇게 처리합니다.)

        hwp.Run('TableRightCellAppend') # 탭 키를 눌러서 아래에 새 행을 생성.원본 문장 수만큼 반복!

    hwp.SaveAs(r"c:\users\jjc\desktop\최종본.hwp")
    hwp.Quit()
